...
website-main/
├── index.html                 # TOPページ
├── site.css                  # 共通スタイル + TOPページ固有スタイル
├── site.js                   # 全ページ共通の初期化スクリプト
├── robots.txt                 # クローラー向けの巡回ルール
├── sitemap.xml                # 検索エンジン向けサイトマップ
├── llms.txt                   # AI/LLM向けのサイト概要
├── activity-contents/
...
JavaScriptはルート直下の `site.js` に集約し、全ページで共通利用します。
...
カラー等の変更は **`site.css` の `:root` 変数** を編集してください。
各ページ固有の見た目は `site.css` / `activity-contents/activities.css` /
`hyutte/mountain-hut.css` / `link/links.css` で調整します。
